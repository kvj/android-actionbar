...
public class ActionBar extends RelativeLayout {
	 ...
	 * 
	 * <p>
	 * Due to the nature of screen real estate on a phone, this will only be
	 * displayed when in {@link #NAVIGATION_MODE_STANDARD} and will hide the
	 * flag {@link #DISPLAY_SHOW_TITLE}, if enabled.
	 * </p>
	 * 
	 ...
	 * {@link #DISPLAY_SHOW_CUSTOM} to be disabled.
	 * </p>
	 * 
	 ...
	 */
	public static final int NAVIGATION_MODE_STANDARD = 0x0;// =
	 ...
	 */
	public static final int NAVIGATION_MODE_LIST = 0x1;// =
	 ...
	 */
	public static final int NAVIGATION_MODE_TABS = 0x2;// =
	 ...
	 * 
	 * @param context
	 *            Context to which the action bar will be tied.
	 * @param attrs
	...
	public ActionBar(Context context, AttributeSet attrs) {
		...
		mHomeView = (FrameLayout) mBarView
				.findViewById(R.id.actionbar_home_view);
		...
		mSubtitleView = (TextView) mBarView
		...
		mCustomView = (FrameLayout) mBarView
				.findViewById(R.id.actionbar_custom);
		...
		if (title != null) {
			...
			try {
				ComponentName componentName = ((Activity) context)
						.getComponentName();
				PackageManager packageManager = ((Activity) context)
						.getPackageManager();
				setTitle(packageManager.getActivityInfo(componentName,
	...
	private void reloadDisplay() {
		...
		final boolean hasList = (mListAdapter != null)
		...
		final boolean hasSubtitle = (mSubtitleView.getText() != null)
				&& !mSubtitleView.getText().equals("");

		if (getDisplayOptionValue(DISPLAY_SHOW_HOME)) {
			mHomeUpIndicator
					.setVisibility(getDisplayOptionValue(DISPLAY_HOME_AS_UP) ? View.VISIBLE
							: View.GONE);
		...
		if (isList) {
			...
			if (hasList) {
				mListView.addView(mListAdapter.getView(mSelectedIndex, oldView,
						mListView));
			}
		...
		mListIndicator.setVisibility(isList && hasList ? View.VISIBLE
				: View.GONE);
	 ...
	 * 
	 * <p>
	 * The display option DISPLAY_SHOW_CUSTOM must be set for the custom view to
	 * be displayed.
	 * </p>
	 * 
	 * @param resId
	 ...
	 * 
	 * @param showHomeAsUp
	 *            {@code true} to show the user that selecting home will return
	 ...
	 * 
	 * <p>
	 * Example: {@code setDisplayOptions(0, DISPLAY_SHOW_HOME)} will disable the
	 ...
	 * 
	 * @param showCustom
	 *            {@code true} if the currently set custom view should be
	 ...
	 * 
	 * @param showHome
	 *            {@code true} to show home, {@code false} otherwise.
	 * 
	 * @see #setDisplayOptions(int)
	 * @see #setDisplayOptions(int, int)
	 */
	 ...
	 * 
	 * @param showTitle
	 *            {@code true} to display a title if present.
	 * 
	 * @see #setDisplayOptions(int)
	 * @see #setDisplayOptions(int, int)
	 */
	 ...
	 * 
	 * @param useLogo
	 *            {@code true} to use the activity logo, {@code false} to use
	 *            the activity icon.
	 * 
	 * @see #setDisplayOptions(int)
	 * @see #setDisplayOptions(int, int)
	 ...
	 * 
	 * @param adapter
	 *            An adapter that will provide a list of items to be rendered to
	 *            string.
	 * @param callback
	...
	public void setNavigationMode(int mode) {
		if ((mode != NAVIGATION_MODE_STANDARD)
				&& (mode != NAVIGATION_MODE_LIST)
				&& (mode != NAVIGATION_MODE_TABS)) {
			throw new IllegalArgumentException("Unknown navigation mode value "
	 ...
	 * 
	 * <p>
	 * To add a home action using this method make sure the action ID is
	 * {@code R.id.actionbar_item_home}.
	 * </p>
	 * 
	 * @param action
	 *            Action to add.
	...
	public void addAction(Action action, int index) {
		if (!action.mActionBar.equals(this)) {
			throw new IllegalStateException(
					"Cannot add an action from a different action bar.");
		}
	 ...
	 * 
	 * @param index
	 *            Position of the action to remove.
	 */
	...
	/**
	 * <p>
	 * Set a logo for the action bar.
	 * </p>
	 * 
	 * <p>
	 * You must call {@link #setDisplayUseLogoEnabled(boolean)} or either
	 * {@link #setDisplayOptions(int)} or {@link #setDisplayOptions(int, int)}
	 * with the {@link #DISPLAY_USE_LOGO} flag.
	 * </p>
	 * 
	 * <p>
	 * <em>Note:</em> For forward compatibility you should also specify your
	 * logo in the {@code android:logo} attribute of the entry for the activity
	 * and/or the application in the manifest.
	 * </p>
	 * 
	 * @param resId
	 *            Resource ID of the logo.
	 * 
	...
	/**
	 * Exception which conveys a particular feature or method is not
	 * implemented.
	 */
	...
	public static class Action implements MenuItem {
		...
		Action(ActionBar actionBar) {
			...
			mView = mActionBar.mInflater.inflate(R.layout.actionbar_item,
					mActionBar.mActionsView, false);
			mView.setTag(this);
		...
		public final Action setIcon(int iconRes) {
			Drawable icon = mActionBar.getContext().getResources()
					.getDrawable(iconRes);
			mIcon.setImageDrawable(icon);
			return this;
		}
		...
		public final Action setOnMenuItemClickListener(
		...
		public final Action setTitle(int title) {
			if (title > 0) {
				this.setTitle(mActionBar.getContext().getResources()
						.getText(title));
			}
			return this;
		}
		...
		/**
		 * Perform all actions associated with selection. Flip the checked state
		 * (if checkable), execute the {@link OnMenuItemClickListener} (if
		 * available) or the {@link Activity#onMenuItemSelected(int, MenuItem)}
		 * that will call through to
		 * {@link Activity#onOptionsItemSelected(MenuItem)}. The {@link Intent}
		 * will always be executed (if set).
		 */
		public void select() {
			if (mIsCheckable) {
			...
			if (mListener != null) {
				mListener.onMenuItemClick(this);
					...
					&& mActionBar.getContext() instanceof Activity) {
	...
	final class MenuImpl implements Menu {
		...
		public Action add(CharSequence title) {
			return this.add(DEFAULT_GROUP_ID, DEFAULT_ITEM_ID, DEFAULT_ORDER,
					title);
		}
		...
		public int addIntentOptions(int groupId, int itemId, int order,
			...
			for (int i = 0; i < count; i++) {
				...
				final Action item = add(groupId, itemId, order,
	...
	public final class Tab {
		...
		Tab() {
			...
			mCustomView = (FrameLayout) mView
					.findViewById(R.id.actionbar_tab_custom);
		}
		...
		void reloadDisplay() {
			...
			this.mCustomView
					.setVisibility(hasCustom ? View.VISIBLE : View.GONE);
		}
		 ...
		 * 
		 * @param text
		 *            Resource ID of text to display.
		 * @return The current instance for call chaining.
		 */
		...
		/**
		 * Unselect this tab. Only valid if the tab has been added to the action
		 * bar and was previously selected.
		 */
		void unselect() {
			if (mIsSelected) {
